...
export default function Page({ params }: IPage) {
  ...
  const [historyList, setHistoryList] = useState([] as IReport[]);
  ...
  useEffect(() => {
    ...
    axios.get("/api/history-list", {
      ...
      let historyList: IReport[] = res.data.history;
      const removeStr = ['log_report_', 'TODO'];
      historyList = historyList.map(item => {
        item.folderNameDisplay = item.folderName;
        removeStr.forEach(str => {
          item.folderNameDisplay = item.folderNameDisplay.replace(str, '')
        });
        return item;
      })
      ...
      if (historyList.length === 0) {
        ...
        setSelectedDate(res.data.history[0].folderName);
      }
    });
  ...
  return (
    <>
      <div className="dashboard-page-component container mx-auto mt-10">
        ...
        <h1 className={`${haveReportsToday ? "hidden" : "hidden"} items-center justify-center text-yellow-200`}> Did you forget to insert the test data for {currentDay}? If not, please click
                ...
                <ListGroup>
                  {
                    historyList.map(value =>
                      <ListGroup.Item
                        key={value.id}
                        active={selectedDate === value.folderName}
                        onClick={(() => { setSelectedDate(value.folderName) })}
                      >
                        {value.folderNameDisplay}
                      </ListGroup.Item>
                    )
                  }
              ...
              <form className="flex flex-col gap-4">
                <div>
                  ...
                  <TextInput
                    id="date"
                    type="text"
                    placeholder="Please input the exact folder name that includes the CSV file."
                    ref={dateRefModal}
                    required={true}
                  />
                </div>
