    ...
    file_list.each {
        ...
        error.each {
            ...
            case
                ...
                when code == "WA_VMOD_ERROR_PRODUCT_NOT_SUPPORTED"
                    if File.readlines(log, :encoding => 'ISO-8859-1').grep(/#{code}/).any?
                        is_failed = true
                        er_detected.append("check download")
                    end
...
def check_all dir
    search_error ["WAAPI_ERROR_INVALID_CREDENTIALS", "WA_VMOD_ERROR_INSTALLATION_FAILED", "FEED_ERROR", "WA_VMOD_ERROR_PRODUCT_NOT_SUPPORTED"], dir
end
...
ARGV.each_index { |index|
  case
    ...
    when ARGV[index] =~ /--check-download/i then search_error ["WA_VMOD_ERROR_PRODUCT_NOT_SUPPORTED"], "."
